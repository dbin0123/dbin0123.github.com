...
---   
#### 仿CryptoJS前端生成的SM2秘钥串加解密java工具类
```java
...
#### 版本区别
- 版本1(生成的秘钥串较短) 兼容CryptoJS v3.1.2生成的秘钥数据进行加解密, 使用版本2无法解密(~~可用于接口,前后的加解数据传输~~);
- 版本2(生成的秘钥串较长) 使用版本1加密的数据无法使用版本2解密,不兼容CryptoJS v3.1.2生成的秘钥串进行加解密(~~可用于接口参数加解密~~);